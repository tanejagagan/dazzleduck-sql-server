...
public interface Indexer {
    ...
    private static String constructInputSql(String sourcePrefix, List<String> sourceFiles, List<String> sourceFields, String timeField) {
        ...
        if (sourcePrefix.isEmpty()) {
            source = sourceFiles.stream().map(s -> String.format("'%s'", s)).collect(Collectors.joining(","));
        } else {
            source = sourceFiles.stream().map(s -> String.format("'%s/%s'", sourcePrefix, s)).collect(Collectors.joining(","));
        }
        String selectCols = String.join(",", sourceFields);
        return "SELECT file_row_number as row_num, filename as file_name, %s, %s FROM read_parquet([%s], filename=true)".formatted(timeField, selectCols, source);
    }
    ...
    static String constructWriteSql(List<String> fieldsForIndexing,
        ...
        var inner = "SELECT row_num, file_name, %s, %s FROM %s".formatted(timeField, fs, inputTable);
        ...
        return "WITH inner0 AS (%s),\n ".formatted(inner) +
                ...
                "inner2 AS (SELECT field, row_num, file_name, %s, UNNEST(tokens) as token FROM inner1), \n".formatted(timeField) +
                "inner3 AS (SELECT field, token, file_name, list(row_num) as row_nums, count(row_num) as count FROM inner2 GROUP BY token, field, file_name),\n" +
                "inner4 AS (SELECT field, token, file_name, CASE WHEN count > 1000 THEN NULL ELSE row_nums END AS row_nums, count FROM inner3)\n" +
                "SELECT * FROM inner4 ORDER BY token, field, file_name";
    }
